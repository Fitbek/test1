...
BASE_ENDPOINT = 'CSSEGISandData/COVID-19/blob/master/csse_covid_19_data/csse_covid_19_daily_reports/'
...
def download_covid_data(**kwargs):
    ...
    if response.status_code == 200:
        save_path = f"/home/polo/airflow_dags{kwargs['exec_date']}.csv"
...
def check_if_table_exists(**kwargs):
    ...
    conn = HiveCliHook(hive_cli_conn_id=kwargs['hive_conn'])
    ...
    if 'OK' in result:
        if table in result:
            ...
            logging.info(f'Table {table} does not exist. Proceed with hive creation.')
...
default_arg = {
    'owner': 'student',
    'email': 'student@some-university.edu',
...
with DAG(
    ...
    move_data_to_hdfs = BashOperator(
        ...
        bash_command="""
            hdfs dfs -mkdir -p /home/polo/csv
            hdfs dfs -copyFromLocal /home/polo/airflow_dags/{exec_date}.csv /home/polo/csv
    ...
    process_data = SparkSubmitOperator(
        ...
        application=os.path.join(main_dag.dag.folder, 'scripts/covid_data_processing.py'),
        conn_id='spark_conn',
    ...
    #   B.Если таблица отсутствует, то нам нужно ее создать.
    ...
    check_if_data_available >> download_data >> move_data_to_hdfs >> process_data >> check_if_hive_table_exists >> [create_hive_table, load_to_hive] >> create_hive_table >> load_to_hive
